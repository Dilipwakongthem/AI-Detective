...
class SoundEngine {
  ...
  /**
   * Soft Click - Realistic mouse click sound
   * Mimics actual mouse button mechanics with multiple frequency components
   */
  playSoftClick(volume) {
    const ctx = this.audioContext;
    const now = ctx.currentTime;

    // Component 1: High-frequency "snap" (mechanical click)
    ...
    snapOsc.frequency.exponentialRampToValueAtTime(1800, now + 0.01);

    snapGain.gain.setValueAtTime(volume * 0.12, now);
    snapGain.gain.exponentialRampToValueAtTime(0.001, now + 0.02);

    snapOsc.start(now);
    snapOsc.stop(now + 0.02);

    // Component 2: Mid-frequency body resonance
    ...
    bodyOsc.frequency.exponentialRampToValueAtTime(400, now + 0.03);

    bodyGain.gain.setValueAtTime(volume * 0.08, now + 0.002);
    bodyGain.gain.exponentialRampToValueAtTime(0.001, now + 0.04);

    bodyOsc.start(now + 0.002);
    bodyOsc.stop(now + 0.04);

    // Component 3: Low-frequency "thud" (physical impact)
    ...
    thudOsc.frequency.exponentialRampToValueAtTime(80, now + 0.025);

    thudGain.gain.setValueAtTime(volume * 0.06, now + 0.003);
    thudGain.gain.exponentialRampToValueAtTime(0.001, now + 0.03);

    thudOsc.start(now + 0.003);
    thudOsc.stop(now + 0.03);

    // Component 4: White noise burst (mechanical friction)
    ...
    noiseGain.gain.setValueAtTime(volume * 0.04, now);
